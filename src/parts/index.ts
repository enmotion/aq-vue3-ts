 ...
 * 2.作为批量自动加载的组件js文件，请都辅以.pg.js为尾缀命名规则
...
const requireSources = import.meta.globEager('./**/*.cp.ts'); // vite模式资源自动加载的方式 全深度遍历符 **
const ComponentName = R.keys(requireSources);
export default ComponentName.map((pathName):{name:string,config:any} => {
  // 获取组件配置
  const componentConfig = requireSources[pathName];
  // 获取相关页面所在文件夹位置,取直接父文夹的 PascalCase为 组件命名
  const componentName = upperFirst(camelCase(pathName.split('/').splice(-2,1)[0]));
  // 输出全局组件 名称与配置对象
  return {
    name:componentName,
