  ...
  <div class="xrow items-end h-45 select-none" 
    @mouseleave="mouseoverIndex = -1">
      <span v-for="(option, index) in options" :key="option.label" class="xcol h-auto overflow-hidden">
        <span class="xrow flex-nowrap items-center cursor-pointer rounded-t-md border-t transition-all duration-200"
          :class="[
            option.value == value ? 'px-10 h-40 bg-gray-100 text-gray-600' : 'px-25 h-40',
