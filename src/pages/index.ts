/* 页面自动加载 模块;
 * 1.所有页面的命名，都以页面js文件的直接父级文件夹为命名样本
...
PageNames.forEach((pagename:string)=>{
    ...
    //获取相关页面所在文件夹位置,取直接父文夹名为页面名称
    let name = upperFirst(camelCase(pagename.split('/').splice(-2,1)[0]));
    // PageRouter = R.mergeAll([PageRouter,{[name]:R.isNil(content.default)?content.default:content}]);
    PageRouter[name] = content.default;
})
